...
Public Sub StretchImage(ByVal Pic As StdPicture)
    ...
    With FrmMp3TagInfo.imgArt
        Horizontal = (Pic.Width < Pic.Height)
        If Horizontal Then
            Div = CDbl(Pic.Width) / CDbl(Pic.Height)
            .Width = FrmMp3TagInfo.picArt.ScaleWidth * Div
            .Height = FrmMp3TagInfo.picArt.ScaleHeight
        Else
            Div = CDbl(Pic.Height) / CDbl(Pic.Width)
            .Width = FrmMp3TagInfo.picArt.ScaleWidth
            .Height = FrmMp3TagInfo.picArt.ScaleHeight * Div
        End If
        .Left = (FrmMp3TagInfo.picArt.ScaleWidth - .Width) / 2
        .Top = (FrmMp3TagInfo.picArt.ScaleHeight - .Height) / 2
        Exit Sub
MyErr:
        .Width = FrmMp3TagInfo.picArt.ScaleWidth
        .Height = FrmMp3TagInfo.picArt.ScaleHeight
...
Public Sub LoadMultiData(Ctl As Object, Frame As MultiFrameData, ByVal Description As String, CountControl As Label, PrevControl As Image, NextControl As Image, DelControl As Image, Col As Collection, Index As Long, Total As Long, FrameBlank As Boolean)
    ...
    If bPic Then
        ...
        With FrmMp3TagInfo.cmbImageType
    ...
    If Col.Count = 0 Then
        ...
        FrameBlank = True
        If bPic Then
            FrmMp3TagInfo.cmbImageType.Enabled = False
            FrmMp3TagInfo.cmbPictureType.Enabled = False
            SetBG False
            Set FrmMp3TagInfo.imgArt.Picture = Nothing
            StretchImage FrmMp3TagInfo.imgArt.Picture
            FrmMp3TagInfo.imgArt.Visible = False
            FrmMp3TagInfo.lblBrowse.Visible = True
            FrmMp3TagInfo.cmbImageType.ListIndex = 2 * (FrmMp3TagInfo.cmbImageType.ListCount \ 4)
            FrmMp3TagInfo.cmbPictureType.ListIndex = 0
            Ctl.ToolTipText = ""
            FrmMp3TagInfo.imgArt.ToolTipText = ""
        Else
            Ctl.Text = ""
        End If
        sNext = I_ADDI: sNextT = ""
    ...
    Else
        ...
        If bPic Then
            FrmMp3TagInfo.cmbImageType.Enabled = True
            FrmMp3TagInfo.cmbPictureType.Enabled = True
            FrmMp3TagInfo.lblBrowse.Visible = False
            FrmMp3TagInfo.imgArt.Visible = True
            Set GPC = New GDIPlusCandy
            Set Pic = GPC.DataToImage(Col(1))
            Set GPC = Nothing
            Set FrmMp3TagInfo.imgArt.Picture = Nothing
            StretchImage Pic
            Set FrmMp3TagInfo.imgArt.Picture = Pic
            SetBG True
            FrmMp3TagInfo.cmbImageType.ListIndex = GetIndex(cAPICIType(1), ID3Revision)
            FrmMp3TagInfo.cmbPictureType.ListIndex = cAPICType(1)
            Ctl.ToolTipText = S_APICTT
            FrmMp3TagInfo.imgArt.ToolTipText = S_APICTT
        Else
            Ctl.Text = Col(1)
        End If
        If Col.Count > 1 Then
    ...
    Set PrevControl.Picture = FrmMp3TagInfo.Buttons.ListImages(I_PREVI).Picture
    PrevControl.ToolTipText = ""
    Set NextControl.Picture = FrmMp3TagInfo.Buttons.ListImages(sNext).Picture
    NextControl.ToolTipText = sNextT
    Set DelControl.Picture = FrmMp3TagInfo.Buttons.ListImages(sDel).Picture
